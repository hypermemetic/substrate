...
| Limitation | Description |
|------------|-------------|
| **No IR integration** | Templates generated from raw JSON Schema, not IR |
| **Flat output** | `schemaToMustache` doesn't generate structured templates |
| **Dead code** | `OutputMode` enum unused |
| **Placeholder templates** | Many templates are just `{{.}}` |
| **No variant awareness** | Tagged unions rendered as flat objects |
>>>>>>> eec8e46b (docs: add template system architecture with dead code analysis)
...
### 2. IR-Driven Template Generation

Generate templates from IR instead of raw JSON Schema:

```haskell
-- Current (schema-based)
schemaToMustache :: Schema -> Text

-- Proposed (IR-based)
typeDefToMustache :: TypeDef -> Text
```

Benefits:
- Access to field names and types
- Structured iteration for arrays
- Conditional sections for optional fields

### 3. Structured Generation

Generate multi-line templates with proper sections:
...
### 4. Tagged Union Support

Render variants appropriately:
